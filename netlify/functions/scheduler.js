...
await dbx.filesListFolder({path:`/SMM/Instagram`},{include_deleted:false,limit:256}).then(async (response) => {
...
try {
return {statusCode:200,body:JSON.stringify({success:true,file:fyl})};
} catch (error) {
    console.log(error);
    return {
      statusCode: 500,
